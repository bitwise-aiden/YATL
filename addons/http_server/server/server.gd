...
# Constant variables
const BAD_REQUEST: String = "HTTP/1.1 400 BAD_REQUEST\r\n\r\n"
...
func listen(port: int = 3000) -> void:
	if self.__server == null:
		print(
			"[ERR] Server not initialized, please initialize before calling `listen`"
		)
		return
